...
func TestADBListApps(t *testing.T) {
	...
	got, err := NewADB(r).ListApps(withTimeout(t), "emulator-5554")
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 2 {
...
func TestADBListAppsUsageAndTool(t *testing.T) {
	...
	if _, err := adb.ListApps(withTimeout(t), ""); !errors.Is(err, ErrUsage) {
		t.Fatalf("empty serial %v", err)
	}
	_, err := adb.ListApps(withTimeout(t), "serial")
