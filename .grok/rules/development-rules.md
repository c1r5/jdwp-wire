...
Ler `CONTEXT.md` antes de abrir issue.
...
- Mudança de `CONTEXT.md` que altera corte MVP/v1 também ganha issue.
...
- Atualizar `CONTEXT.md` no PR que muda corte MVP/v1 ou contrato de CLI.
...
17. **Compat de flag.** Flag nova é ok. Renomear/remover flag = issue + menção no CONTEXT.
